...
class TestCourier:
    ...
    class TestLoginCourierNegative:
        ...
        def test_login_courier_with_incorrect_login(
                                                                ...
                                                                class_courier: CourierFixture
                                                                ):
            request = LoginCourierRequestSchema(login=fake.login(), password=class_courier.request.password)
            response = courier_client.login_courier_api(request=request)
        ...
        def test_login_courier_with_incorrect_password(
            ...
            request = LoginCourierRequestSchema(login=class_courier.request.login, password=fake.password())
            response = courier_client.login_courier_api(request=request)
            response_data = LoginCourierNotFoundResponseSchema.model_validate_json(response.text)
        ...
        def test_login_courier_without_required_parametres(self, courier_client: CourierClient, login, password):
            request = LoginCourierBadRequestSchema(login=login, password=password)
            response = courier_client.login_courier_api(request=request)
